...
if __name__ == """__main__""":
    ...
    if 1 == 1:
        ...
        train_RDD = sc.textFile(folder_path+"train1.csv").filter(lambda x: not x.startswith("user_id")).map(
            read_train) #yelp_train.csv
    ...
    if 1 == 1:
        # read user.json
        user_data = []
        with open(folder_path + "user1.json") as in_file: #"user.json"
            while True:
